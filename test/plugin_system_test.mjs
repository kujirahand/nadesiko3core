...
describe('plugin_system_test', () => {
  ...
  it('掛けるの拡張', () => {
    cmp('2に3を掛けて表示', '6')
    cmp('"z"に3を掛けて表示', 'zzz')
    cmp('[0]に3を掛けてJSONエンコードして表示', '[0,0,0]')
  })
  it('JS実行', () => {
    cmp('「3+6」をJS実行して表示', '9')
  ...
  it('切り取る', () => {
    ...
    // #1096
    cmp('「abc,def」から「,」まで切り取る。対象を表示。', 'def')
    cmp('「abc,def」から「*」まで切り取る。対象を表示。', '')
  })
  it('文字削除', () => {
  ...
  it('置換', () => {
    ...
    cmp('「番号00▲00▲」の「00」を「11」に置換して表示', '番号11▲11▲')
  })
  it('単置換', () => { // 単置換の問題 #1105
    cmp('「e,f,g」の「,」を「-」へ単置換して表示。', 'e-f,g')
    cmp('「番号0000」の「0000」を「0001」に単置換して表示', '番号0001')
    cmp('「あい▲うえ▲お」の「▲」を「＊」に単置換して表示', 'あい＊うえ▲お')
    cmp('「あい▲▲うえ▲▲お」の「▲▲」を「＊」に単置換して表示', 'あい＊うえ▲▲お')
  })
  it('空白除去', () => {
  ...
  it('ゼロ埋め', () => {
    cmp('3を3でゼロ埋め。表示。', '003')
    cmp('33を3でゼロ埋め。表示。', '033')
    cmp('333を3でゼロ埋め。表示。', '333')
    cmp('3333を3でゼロ埋め。表示。', '3333')
  ...
  it('表曖昧検索', () => {
    ...
    cmp('A=[[1,"佐藤"],[2,"加藤"],[3,"佐々木"]];Aの1から1で「佐」を表曖昧検索して表示。', '2')
  })
  it('表正規表現ピックアップ', () => {
  ...
  it('プラグイン一覧取得', () => {
    cmp('プラグイン一覧取得して「:」で配列結合して表示', 'PluginSystem:PluginMath:PluginPromise:PluginAssert:PluginCSV')
  })
  it('配列切り取り', () => {
  ...
  it('文字列分割 #1098', () => {
    cmp('「あ::い::う」を「::」で文字列分割してAに代入; A[0]を表示。A[1]を表示。', 'あ\nい::う')
    cmp('「000-111-222」を「-」で文字列分割してAに代入; A[0]を表示。A[1]を表示。', '000\n111-222')
    cmp('「ああ,いい,うう」を「,」で文字列分割して「==」で配列結合して表示。', 'ああ==いい,うう')
    cmp('「ああ::いい::うう」を「::」で文字列分割して「=」で配列結合して表示。', 'ああ=いい::うう')
  })
  it('出現回数 #1105', () => {
    cmp('「あ::い::う」で「::」の出現回数を表示。', '2')
    cmp('「あ::い::う::」で「::」の出現回数を表示。', '3')
    cmp('「::あ::い::う」で「::」の出現回数を表示。', '3')
    cmp('「▲あい▲うえ▲お」で「▲」の出現回数を表示。', '3')
    cmp('「あい▲▲うえ▲▲お」で「▲▲」の出現回数を表示。', '2')
    cmp('「番号0000」で「00」の出現回数を表示。', '2')
    cmp('「番号0001」で「0」の出現回数を表示。', '3')
  })
  it('日時処理(簡易) #1117', () => {
    cmp('「2021/12/25」の曜日を表示。', '土')
    cmp('(1640393988825/1000)を日時変換を表示。', '2021/12/25 09:59:48')
    cmp('「2021/01/01」を和暦変換して表示。', '令和3年01月01日')
    cmp('「1926/12/29」を和暦変換して表示。', '昭和元年12月29日')
    cmp('「1990/03/30」を和暦変換して表示。', '平成2年03月30日')
  ...
  it('日時加算 #1117', () => {
    cmp('「2017/03/06 00:00:01」に「+00:00:00」を時間加算。それを表示', '2017/03/06 00:00:01')
    ...
    cmp('「2022/02/16 00:00:02」に「-2秒」を日時加算。それを表示', '2022/02/16 00:00:00')
    cmp('「2022/02/16 00:02:00」に「-2分」を日時加算。それを表示', '2022/02/16 00:00:00')
    cmp('「2022/02/16 02:00:00」に「-2時間」を日時加算。それを表示', '2022/02/16 00:00:00')
    cmp('「2017/03/06 00:00:01」に「-3秒」を日時加算。それを表示', '2017/03/05 23:59:58')
    cmp('「2022/02/16」に「-1日」を日時加算。それを表示', '2022/02/15')
    ...
    cmp('「2021/12/01」に「2週間」を日時加算して表示', '2021/12/15')
    cmp('「2021/12/29 23:59:59」に「2週間」を日時加算して表示', '2022/01/12 23:59:59')
  })
  it('日時書式変換 #1117', () => {
    cmp('T=「2000/1/2 00:00:00」をUNIX時間変換;Tを「YYYY-MM-DD」で日時書式変換して表示', '2000-01-02')
    cmp('T=「2000/1/2 03:04:05」をUNIX時間変換;Tを「YYYY-MM-DD HH:mm:ss」で日時書式変換して表示', '2000-01-02 03:04:05')
    cmp('T=「2021/12/25」をUNIX時間変換;Tを「YYYY-MM-DD(W)」で日時書式変換して表示', '2021-12-25(土)')
    cmp('T=「2021/12/25」をUNIX時間変換;Tを「YYYY-MM-DD(WWW)」で日時書式変換して表示', '2021-12-25(Sat)')
    cmp('T=「2021/12/25」をUNIX時間変換;Tを「WWW MMM DD YYYY」で日時書式変換して表示', 'Sat Dec 25 2021')
    cmp('1640438681.025を「HH:mm:ss.ccc」で日時書式変換して表示', '22:24:41.025')
  })
  it('日時書式変換 #1121', () => {
    cmp('「2000/1/2 00:00:00」を「YYYY-MM-DD」で日時書式変換して表示', '2000-01-02')
    cmp('「2023/4/5 00:00:00」を「YYYYMMDD」で日時書式変換して表示', '20230405')
    cmp('1640438681.025を「HH:mm:ss.ccc」で日時書式変換して表示', '22:24:41.025')
  })
  it('『昨日』の問題 #1124', () => {
    cmp('昨日。もしそれが「」でなければ「OK」と表示', 'OK')
    cmp('今日に「-1日」を日時加算してDに代入。昨日とDが等しい。もしそうならば「OK」と表示。', 'OK')
    cmp('今日に「1日」を日時加算してDに代入。明日とDが等しい。もしそうならば「OK」と表示。', 'OK')
    cmp('今月に1を足してDに代入。来月とDが等しい。もしそうならば「OK」と表示。', 'OK')
    cmp('今月に-1を足してDに代入。先月とDが等しい。もしそうならば「OK」と表示。', 'OK')
    cmp('今年に1を足してDに代入。来年とDが等しい。もしそうならば「OK」と表示。', 'OK')
    cmp('今年に-1を足してDに代入。去年とDが等しい。もしそうならば「OK」と表示。', 'OK')
  })
  it('『二乗』 #1140', () => {
    cmp('3の二乗を表示', '9')
    cmp('2の3のべき乗を表示', '8')
